...
class ColorblindFilters {
    ...
    public static function applyFiltersOnGame() {
        ...
        if (FlxG.save.data.ColorBlindType != "None") { // actually self explanatory, isn't it?
            if (filterMap.get(FlxG.save.data.ColorBlindType) != null) { // anticrash system
                var thisF = filterMap.get(FlxG.save.data.ColorBlindType).filter;
                if (thisF != null) {
