...
class MainMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		for (i in 0...optionShit.length)
		{
			...
			menuItem.frames = tex;
			menuItem.animation.addByPrefix('idle', (currentOptionShit == 'freeplay glitch' ? 'modifiers' : currentOptionShit) + " basic", 24);
			menuItem.animation.addByPrefix('selected', (currentOptionShit == 'freeplay glitch' ? 'modifiers' : currentOptionShit) + " white", 24);
						...
						else
						{
							FlxFlicker.flicker(spr, 1, 0.06, false, false, function(flick:FlxFlicker)
							{
								...
								switch (daChoice)
								{
									...
									case 'modifiers' | 'freeplay glitch':
										if (FlxG.random.bool(0.05))
										{
											fancyOpenURL("https://www.youtube.com/watch?v=Z7wWa1G9_30%22"); // what
										}
										if (!awaitingExploitation) {
